...
- **品質把關（MUST）**：§4-17 規定內容產線唯一的常態人工檢查點是課綱定稿，這 800～1,200 道題不會有人逐題審，而結構性檢查攔不住「正解標錯」。故 Gate 除結構檢查外 MUST 對每題執行**獨立二次作答交叉驗證**（build-time 以獨立 LLM 呼叫盲答、**不提供標記的正解**，不一致者丟棄重生）。此機制非 100% 覆蓋（同模型家族有相關性錯誤），MUST NOT 被描述為正確性保證。**per-Concept 內容重生上限 3 輪**；**交叉驗證呼叫本身的基礎設施失敗（API 錯誤／逾時／回應無法解析）MUST 走 §20.4 既有的節流與 429 退避重試，MUST NOT 計入該 3 輪上限**——否則網路抖動會被誤判為「這個 Concept 出不出好題」而觸發具名失敗。
  ...
  **結構性 Gate MUST 另含兩條「猜答偏誤」的 per-Concept 集合層判準**（2026-08-07 實測後新增）：`quiz-answer-position-bias`（任一 `answerIndex` 佔比 MUST ≤ 50%）與 `quiz-longest-option-bias`（「正解恰為該題唯一最長選項」的題數佔比 MUST ≤ 50%），題數 < 4 時不套用。**理由**：實測某 Concept 的 10 題全數通過當時既有的 9 條判準，卻有 80% 正解落在 B、90% 正解是唯一最長選項——學習者「一律選最長的 B」即得 80 分而不必理解內容；同批 31 個 Concept 複驗為 **31/31 違規**，屬系統性偏誤而非個案。本 Feature 的價值全在**誠實的自我訊號**，此類題目量測不到任何東西卻無任何徵兆，正是憲章 XV 要防的靜默失效。**MUST NOT 只靠 prompt 敘述防範**（§20.4 已實證敘述性要求不可靠）；門檻 **MUST NOT 收緊至接近隨機期望值 25%**，否則正常波動即觸發重生、白燒免費層額度。
