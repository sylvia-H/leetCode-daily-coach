...
1. **修復目標 = Gate 上限（`floor(n/2)`）再少一題**，即 `longestBiasRepairTarget(n)`。
   修復題數 = `offenders − longestBiasRepairTarget(n)`。
   ...
   **MUST NOT 等於 Gate 上限**：修復是 best-effort，重出的題仍是唯一最長時保留原題。目標等於上限
   ⇒ **任一題修復失敗即立刻超標、整輪重來**（約 10 次呼叫），遠貴於多修一題（2 次呼叫）。
   `round(n × 0.4)` 在 n=5/7/9 恰好等於上限，故 MUST NOT 直接用百分比。

   **MUST NOT 收緊到隨機基準 25%**：修復只在**已超標**時啟動，而整體長度偏誤率僅 33.6%
   （實測 2026-08-07 前 37 個 Concept），觸發時多為擦邊（4/7、5/8），多修的那一兩題大多是白花的
   ——n=8／offenders=5／單題修復成功率 0.7 時，期望成本約 6.3 次 vs. 本設定的 4.9 次。

   殘留佔比落在 20%～40%，離「正解**從不**最長」的反向線索（0%，與「恆為最長」同樣可被利用，
   只是方向相反）與 Gate 上限（50%）都有距離。
