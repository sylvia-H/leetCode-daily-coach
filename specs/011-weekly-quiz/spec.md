...
- **FR-010b**（2026-08-07 實測後新增）: 測驗素材 MUST 通過三條**猜答偏誤**的結構性判準，皆為 per-Concept 的**集合層**統計，題數 < 4 時不套用（樣本太小佔比無統計意義）：
  ...
  **`quiz-longest-option-bias` 的重生顆粒度 MUST 為逐題，MUST NOT 為整個 Concept**（quiz-bank-schema.md §5.2b）：整輪重生會把已通過交叉驗證的好題一起丟掉，實測全量跑至第 37 個 Concept 時 **19/37（51%）需要 2 輪以上**，成本主因即此。改為只重出「正解為唯一最長」的題（複用交叉驗證失敗時既有的 per-aspect 重出機制），一輪從約 10 次呼叫降至約 4 次。**修復目標 MUST 為「Gate 上限（`floor(n/2)`）再少一題」，MUST NOT 等於 Gate 上限、亦 MUST NOT 收緊到隨機基準 25%**。等於上限 ⇒ best-effort 的修復只要有一題失敗就立刻超標、整輪重來（約 10 次呼叫），遠貴於多修一題（2 次呼叫）；`round(n × 0.4)` 在 n=5/7/9 恰好等於上限，故 MUST NOT 直接用百分比表述。反向地，收緊到 25% 也不划算——修復只在已超標時啟動，而整體長度偏誤率僅 33.6%，觸發時多為擦邊（4/7、5/8），多修的題大多白花（n=8／offenders=5／單題成功率 0.7 時期望成本 6.3 次 vs. 4.9 次）。最終殘留佔比落在 20%～40%，離「正解**從不**最長」的反向線索（0%，與「恆為最長」同樣可被利用）與 Gate 上限（50%）都有距離。修復為 best-effort：替換題不符要求即保留原題，**Gate 仍是權威**，修完仍超標即照既有路徑判本輪不通過。
