...
> **計數口徑更新（2026-08-07 第二次 `/speckit-analyze`）**：本檔上文各處提到的「8 條 Gate 規則」
> 為當時口徑；其後補上 FR-010 早已要求卻漏列的 `quiz-leetcode-id`（無 LeetCode 題號／題目連結），
> 現行為 **9 條**（8 條由 `checkQuizBank` 輸出 + `quiz-schema` 由載入層 throw）。
> 本檔各項「8 條中無一檢查 X」的結論**不受影響**——新增的第 9 條同樣不涉及面向覆蓋或語意重複。
