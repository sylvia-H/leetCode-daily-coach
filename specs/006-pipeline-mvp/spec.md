...
- Q（code review 後補問，2026-07-29）: 三軌在 F7 課綱進來前必然完課，而 F7 把課表由各 13 課延長到約
  180 課後，殘留的完課時間會讓三軌**永久靜默**——要靠 runbook 提醒人工清除、fail loud 報錯，還是程式
  自動解除？ → A: **程式自動解除**（新增 FR-022b，回寫 `docs/spec.md` §9.2 / §19，並取代原「程式 MUST NOT
  自動清除」的裁決）。理由：①「完課時間非空 ⇒ 進度超出課表最大 `sessionIndex`」是既有的狀態不變式，
  課表延長後該狀態已自我矛盾，解除是修復不一致而非猜測意圖；②完課判定本就在 `main.ts` 進行、手上就有
  課表，原裁決「狀態層不認識課表」的理由不適用於此處；③停用某軌的正規手段是移除 webhook 設定
  （FR-007），完課時間不是人工暫停開關，自動解除不會誤傷刻意停課；④內容擴充是本專案的必然事件，
  要求每次擴充都做一次人工狀態編輯等同新增一道常態人工關卡（憲章 XVII 的反面）
- Q（同上）: 該軌課表為空（0 個 Session）時應判完課還是失敗？ → A: **該軌失敗**（補入 FR-022 子條與
  Edge Cases）。空課表下「超出最大 `sessionIndex`」恆成立，判完課等於讓生成物異常靜默走進終態並寫入
  完課時間；與既有的「中間缺號 ⇒ 該軌失敗」裁決一致
...
- **該軌課表為空**（`sessions` 為 0 個）：MUST 判為**該軌失敗**，MUST NOT 判為完課（FR-022）——空課表
  下「超出最大 `sessionIndex`」對任何進度都成立，誤判會使產物異常靜默走進終態並寫入完課時間。
- **已完課的 Track 其課表事後被延長**（例：F7 課綱由各 13 課的種子課表展開為完整課綱）：MUST 自動
  清除該軌完課時間並從既有進度續推（FR-022b），MUST NOT 因殘留的完課時間而繼續靜默跳過。
...
- **FR-022**: 某 Track 的 `currentSessionIndex` **超出該軌課表的最大 `sessionIndex`** 時 MUST 視為該軌的
  **完課（終態）**而非失敗（課表**中間缺號**不適用本條，見 Edge Cases，MUST 判為該軌失敗）：
  ...
  - **強制模式 MUST NOT 繞過完課跳過**：強制模式只繞過**日期 guard**；已記錄完課時間的 Track 在強制
    模式下 MUST 仍然跳過。
  - **空課表 MUST NOT 判為完課**：該軌課表的 Session 數為 **0** 時 MUST 判為**該軌失敗**（紅色告警 +
    非零結束狀態）。理由：完課判定式「超出最大 `sessionIndex`」在空課表下對**任何**進度都成立，會使
    課表產物異常靜默走進完課終態並寫入完課時間；空課表與中間缺號同屬生成物異常，處置 MUST 一致。
...
- **FR-022b**（**完課狀態的自動解除**，2026-07-29 定案，回寫 `docs/spec.md` §9.2 / §19）: 已記錄完課
  時間的 Track，若其 `currentSessionIndex` **仍未超出**目前課表的最大 `sessionIndex`（＝課表在完課後
  被延長），MUST **自動清除該軌的完課時間**並於當次照常從既有進度續推；清除 MUST **只刪除該欄位**
  （MUST NOT 更動 `currentSessionIndex` / `lastPushAt` / `history` / `completedConceptIds`），且 MUST 於
  執行記錄留下可辨識的紀錄。預覽模式下 MUST NOT 寫入狀態（只輸出紀錄）。
  - **理由**：完課時間的不變式是「進度已超出課表最大 `sessionIndex`」（FR-022）；課表延長後此不變式
    已被違反，殘留的完課時間不再代表「已完課」。若不解除，F7 課綱由種子課表（各 13 課）展開為完整
    課綱後，**三軌會全部無限期靜默跳過且無任何訊號**——屬憲章 XV 所禁止的沉默失敗，且其成因是內容
    擴充這個必然事件，而非人為誤操作。
  - 本條**取代**「程式一律 MUST NOT 自動清除完課時間」的原裁決（原見 FR-023a / state-schema 契約 §3）。
    自動解除只在「不變式已被違反」時觸發，MUST NOT 擴大為任何形式的進度自動修正。
...
- **FR-023a**（2026-07-29 依 FR-022b 修訂）: runbook MUST 另行明示**已完課 Track 的重新推播程序**：
  把該軌 `currentSessionIndex` 調回課表範圍內，並 SHOULD 一併清除該軌的完課時間欄位（保持狀態檔語意
  一致）。**未清除亦不會導致該軌被靜默跳過**——FR-022b 的自動解除會在下次執行時處理，故 runbook
  MUST NOT 再把此情境描述為「沉默失敗」。runbook MUST 一併說明 FR-022b 的自動解除行為（何時觸發、
  執行記錄長什麼樣），使維運者能在課綱擴充後確認三軌確實恢復推播。
