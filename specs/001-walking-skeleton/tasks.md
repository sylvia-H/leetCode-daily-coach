...
- [X] T022 [P] [US1] 實作 `src/compiler/schedule.ts`：硬編 3-Session 課表與「課程序號 → 前 / 今 / 後觀念名稱」對照表；對外 `getSessionPlan(track, sessionIndex)` / `getPathLabels(sessionIndex)`；序號超出範圍 → 拋**可辨識的「課表用盡」錯誤**。檔頂加註臨時性與接手 Feature（課表 → **F4**、路徑表 → **F5**；原記為 F2，依 F2 clarify 2026-07-21 定案更正為 F5——F2 只建 DAG，接入與移除屬 F5）。資料見 [data-model.md](./data-model.md) §2 / §3
