...
| Phase 11 收批後 | **6%** | 107 |
| Phase 12 收批後 | **12%** | 116 |
| Phase 13 + Phase 14 收批後（一次跑兩批，中途未量測） | **44%** | 135 |
...
| Phase 13 + 14（就地修新制；**orchestrator 為 Fable、單 session 跑兩批**） | 32 | 19 | **1.68** |
...
- **⚠️ 2026-09-02 Phase 13＋14 校準：1.68／Concept（12%→44%、19 個），0.6～0.9 區間被打破，外推上界
  改為 1.7。** 兩批合跑、中途未量測，無法拆分。已知的三個推高因子：
  ...
  3. 同時段 peer session（agent-copilot）在跑，是否用 Fable 未知；儀表板差值不保證全屬本專案。
  **處置**：(a) Phase 15 起外推用 **1.7 當上界**，9～10 個 Concept 的批次估 15～17 個百分點，
  週額度只夠再跑 3 批；(b) 若使用者希望壓回 0.6～0.9，最直接的槓桿是 **orchestrator 改回 Opus**
  （Phase 11 之前的作法，主控 token 不計 Fable）與**一次只跑一批**（session 短、喚醒少）；
  作者的窮舉實測 SHOULD 保留——它是 D10 的第一道防線，砍掉會把偽命題留給 reviewer。
  (c) 本檔先前「就地修制 0.60～0.67」的結論**只在 orchestrator 為 Opus 或 session 很短時成立**，
  MUST NOT 再當通用單價。
