...
在 TypeScript 中，由於沒有原生支援多回傳值的 Tuple 型別作為函式簽章（雖然可以用陣列，但型別推斷較弱），通常會使用特殊標記數值（例如回傳 -1 代表不平衡、非負數代表高度）或是回傳一個帶有明確型別的物件來傳遞狀態。以下為使用 sentinel value -1 的實作範例，內含嚴格的斷言測試確保正確性。
...
if (res !== true) throw new Error("Assertion failed: tree should be balanced");
```

## Python Corner

在 Python 中，支援原生 Tuple 解構賦值，這使得 Bottom-Up Validation 的實作變得極為乾淨且直覺。我們可以讓遞迴函式直接回傳一個二元組 `(is_balanced, height)`，省去使用特殊數值（sentinel value）可能帶來的語意混淆。以下為 Python 版本的實作範例，內含完整的型別標註與斷言驗證。
...
def is_balanced(root: TreeNode | None) -> bool:
    def helper(node: TreeNode | None) -> tuple[bool, int]:
        if not node:
            return True, 0
...
assert is_balanced(root) is True, "Assertion failed: tree should be balanced"
```
