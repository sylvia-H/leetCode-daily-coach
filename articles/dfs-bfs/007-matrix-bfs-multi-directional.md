...
function orangesRotting(grid: number[][]): number {
    ...
    const result = freshCount === 0 ? maxMinutes : -1;
...
def oranges_rotting(grid: list[list[int]]) -> int:
    ...
    result = max_minutes if fresh_count == 0 else -1
    assert result == 4, "assertion failed"
    return result


initial_grid = [[2, 1, 1], [1, 1, 0], [0, 1, 1]]
oranges_rotting(initial_grid)
```
