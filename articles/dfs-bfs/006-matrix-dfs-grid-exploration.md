...
```typescript
function gridDfs(grid: number[][]): number {
...
const result = gridDfs(testGrid);
if (result !== 2) throw new Error("Assertion failed: expected 2 islands");
```
...
assert grid_dfs(test_grid) == 2, "Assertion failed: expected 2 islands"
```
