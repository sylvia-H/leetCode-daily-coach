...
class MinHeap {
  ...
  private down(i: number): void {
    ...
    while ((i << 1) + 1 < len) {
      ...
      if (this.heap[left].val < this.heap[target].val) {
        target = left;
      }
      if (right < len && this.heap[right].val < this.heap[target].val) {
        target = right;
      }
      if (target !== i) {
...
if (count !== 8) throw new Error("assertion failed");
```
...
assert count == 8, "assertion failed"
```
