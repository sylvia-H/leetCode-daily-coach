...
5. **四個選項的長度 MUST 相近**：MUST NOT 把正解寫得比干擾項更完整、更長。實測顯示「正解恆為最長
   選項」是出題時最容易發生的偏誤，會讓學習者只挑最長的就猜對，題目因此量測不到任何理解程度。
   干擾項 MUST 寫得與正解一樣具體、一樣有細節——它們應該是「看起來合理但實際上錯」，而不是
   「一眼就知道太短太隨便」。
6. **正解 MUST 用滿 A／B／C／D 四個位置**：實測顯示模型會習慣性把正解排在靠前的位置——絕大多數
   落在 A，而放在最後一個位置的幾乎不存在——使學習者「一律選 A」就能得高分。每出一題，MUST 先
   看前面各題的正解落在哪，**刻意選一個還沒用過或用得最少的位置**；尤其 MUST NOT 迴避把正解
   放在最後一個位置。
7. **MUST NOT 用「語氣絕對」當作干擾項的唯一破綻**：實測顯示干擾項大量出現「完全不需要」「必然」
   ...
   MAY 用在正解上。
8. **題幹 MUST 精簡、MUST NOT 洩漏答案**：題幹 MUST NOT 複述正解的內容（實測有題目問「為什麼必須
...
9. MUST NOT 在題幹、選項或詳解中提及任何 LeetCode 題號或題目連結——這是自製選擇題，不是 LeetCode
   題目改寫。複雜度標記（如 O(n²)）、陣列索引、題目情境中的數值可以正常出現，這不是題號。
10. MUST NOT 考核任何程式語言 API 用法或語法細節；MUST 考核可遷移的觀念性理解。
11. 全文以繁體中文撰寫；技術術語、Pattern 名稱、程式碼片段 MUST 保留英文原文（§11）。
12. MUST NOT 提及任何題數或面向數字（含上限）——出多少題完全由規則 1 的逐項覆蓋自然決定。
13. 每一題的 aspect 欄位 MUST 逐字等於上方面向清單中的一項，供後續系統對應該題所屬面向。
14. 回傳格式 MUST 為單一 JSON 物件：{ "items": [{ "stem": string, "options": string[4],
