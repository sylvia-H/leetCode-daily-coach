...
5. **四個選項的長度 MUST 相近**：MUST NOT 把正解寫得比干擾項更完整、更長。實測顯示「正解恆為最長
   ...
   「一眼就知道太短太隨便」。
6. **正解 MUST 平均分散在四個位置**：MUST NOT 讓多數題目的正解都落在同一個位置。決定正解要放第幾個
   位置時，MUST 刻意與前幾題錯開，而不是習慣性地放在同一格。
7. MUST NOT 在題幹、選項或詳解中提及任何 LeetCode 題號或題目連結——這是自製選擇題，不是 LeetCode
   題目改寫。複雜度標記（如 O(n²)）、陣列索引、題目情境中的數值可以正常出現，這不是題號。
8. MUST NOT 考核任何程式語言 API 用法或語法細節；MUST 考核可遷移的觀念性理解。
9. 全文以繁體中文撰寫；技術術語、Pattern 名稱、程式碼片段 MUST 保留英文原文（§11）。
10. MUST NOT 提及任何題數或面向數字（含上限）——出多少題完全由規則 1 的逐項覆蓋自然決定。
11. 每一題的 aspect 欄位 MUST 逐字等於上方面向清單中的一項，供後續系統對應該題所屬面向。
12. 回傳格式 MUST 為單一 JSON 物件：{ "items": [{ "stem": string, "options": string[4],
