...
describe("compile — 三軌共用同一份教材正文（US5、SC-005、憲章 VI）", () => {
  ...
  it("同一 conceptId 於三個 Track 編譯，concept 的教材欄位全等", () => {
    const foundation = asConcept(compile("foundation", 4, deps));
    const interviewReady = asConcept(compile("interviewReady", 4, deps));
    const interviewMastery = asConcept(compile("interviewMastery", 4, deps));
